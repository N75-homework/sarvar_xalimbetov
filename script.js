...
let $selectFilt = document.createElement('select')
let $btn = document.createElement('button')
let $filmsWrap = document.createElement('div')
...
$selectGenre.className = "genre_select"
$selectGenre.name = "genre select"

$selectFilt.className = "filt_select"
$selectFilt.name = "filt select"
...
films.forEach((item) => {
	...
	sortGenresArray.push(getGenres)
})

let genresArray = [...new Set(sortGenresArray)];
genresArray.unshift('All')
...
$input.addEventListener('keyup', e => {
	...
	films.forEach(item => {
		...
		if (item.Title.toLowerCase().trim().includes($input.value.toLowerCase().trim())) {
...
$selectGenre.addEventListener('change', e => {
	...
	films.forEach((item) => {
		...
		} else if (item.genres.includes(e.target.value)) {
			var $item = document.createElement('li')
			$item.className = 'item'
...
let getFilt;
let filtArray = ['Filter', 'A - Z', 'Z - A', 'New - Old', 'Old - New'];


filtArray.forEach((item) => {
	let $optionFilt = document.createElement('option')
	$optionFilt.innerText = item
	$optionFilt.value = item
	$selectFilt.append($optionFilt)
	
})


// const sortOption1 = () => {
//     let result = films.sort((a, b) => a.Year - b.Year)
// 	result.
// 	console.log(result);
...
// $selectFilt.onchange = 'sortOption1();'

$selectFilt.addEventListener('change', () => {
	$list.innerHTML = null

	
	if($selectFilt[1].selected){
		let result = films.sort((a, b) => a.Title > b.Title ? 1 : -1)
		result.forEach(item => {
			var $item = document.createElement('li')
			$item.className = 'item'
	...
	else if($selectFilt[3].selected){
		let result = films.sort((a, b) => b.Year - a.Year)
		result.forEach(item => {
			var $item = document.createElement('li')
			$item.className = 'item'
	...
	else if($selectFilt[0].selected){
		let result = films.sort((a, b) => a.id - b.id)
		result.forEach(item => {
			var $item = document.createElement('li')
			$item.className = 'item'
...
// 	filmTitles = item.Title
// 	sortFilms.push(filmTitles)
// 	sortFilms.sort()

// 	for(let i=0; i<sortFilms.length; i++){

// 		if(sortFilms[i] == item.Title[i]){
// 			console.log(
...
$form.append($input, $icon, $selectGenre, $selectFilt)
$root.append($title, $line, $form, $list)
